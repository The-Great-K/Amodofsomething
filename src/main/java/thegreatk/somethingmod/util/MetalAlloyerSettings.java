...
public class MetalAlloyerSettings {
	...
	public MetalAlloyerSettings() {
		settings = new int[] { 0, 0, 0, 0, 0, 0 };
		autoIO = new int[] { 0, 0 };
		// (mode 1, 2, 3, 4, subtract) ignored, low/high, comparator, comparator sub,
		// subtract
	...
	public int get(int index) {
		try {
			...
			if (Config.showErrors.get()) {
				ModOfSomething.LOGGER.error("Something went wrong.");
				for (int i = 0; i < e.getStackTrace().length; i++) {
					ModOfSomething.LOGGER.error(e.getStackTrace()[i].toString());
				}
			}
		}
		return 0;
